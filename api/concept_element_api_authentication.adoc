...
summary: 使用 API 时，您可以通过在所有 API 请求中包含 HTTP 基本身份验证标头来对系统进行身份验证。如果省略身份验证信息，系统将拒绝未经身份验证的请求，并返回 HTTP 401 响应。该系统支持基于TLS的HTTP基本身份验证。 
---
= API 身份验证
:allow-uri-read: 
:icons: font
...
[role="lead"]
使用 API 时，您可以通过在所有 API 请求中包含 HTTP 基本身份验证标头来对系统进行身份验证。如果省略身份验证信息，系统将拒绝未经身份验证的请求，并返回 HTTP 401 响应。该系统支持基于TLS的HTTP基本身份验证。
