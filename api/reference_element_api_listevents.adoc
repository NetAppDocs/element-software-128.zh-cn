...
summary: 您可以使用 ListEvents 方法列出集群上检测到的事件，并按从旧到新的顺序排序。 
---
= 事件列表
:allow-uri-read: 
:icons: font
...
[role="lead"]
你可以使用 `ListEvents`列出集群上检测到的事件的方法，按从旧到新的顺序排序。
...
事件类型
 a| 
指定要返回的事件类型。看xref:reference_element_api_event.adoc[event]可能的事件类型。
 a| 
string
 a| 
0
...
事件列表。
 a| 
xref:reference_element_api_event.adoc[event]大批
