...
summary: 您可以使用 TestRemoteConnectivity 方法来 ping 远程集群的每个节点，并检查远程集群数据库连接。必须将聚类结果成对才能使用此方法获得有用的结果。如果远程数据库连接失败，系统响应会列出异常情况。 
---
= 测试远程连接
:allow-uri-read: 
:icons: font
...
[role="lead"]
您可以使用 `TestRemoteConnectivity`ping 远程集群的每个节点并检查远程集群数据库连接的方法。必须将聚类结果成对才能使用此方法获得有用的结果。如果远程数据库连接失败，系统响应会列出异常情况。
