...
summary: 您可以使用集成备份功能备份卷快照。您可以将SolidFire集群中的快照备份到外部对象存储，或者备份到另一个SolidFire集群。将快照备份到外部对象存储时，必须与该对象存储建立允许读/写操作的连接。 
---
= 备份卷快照
:allow-uri-read: 
:icons: font
...
* link:task_data_protection_back_up_volume_to_solidfire["将卷快照备份到SolidFire集群"]
