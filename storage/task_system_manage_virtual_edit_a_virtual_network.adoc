...
summary: 您可以更改 VLAN 属性，例如 VLAN 名称、子网掩码和 IP 地址块大小。 VLAN 的 VLAN 标签和 SVIP 不能修改。对于非 VRF VLAN，网关属性不是有效参数。 
---
= 编辑虚拟网络
:allow-uri-read: 
...
* https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/Element_Software/Duplicate_IP_warning_after_adding_a_storage_node_in_VLAN_on_Element_cluster["在 Element 集群的 VLAN 中添加存储节点后出现重复 IP 警告"^]
* https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/NetApp_HCI/How_to_determine_which_VLAN_IP%27s_are_in_use_and_which_nodes_those_IP%27s_are_assigned_to_in_Element["如何在 Element 中确定哪些 VLAN IP 地址正在使用以及这些 IP 地址分配给了哪些节点"^]
